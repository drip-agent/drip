...
function buildActivityMap() {
  ...
  for (const u of UPDATES) {
    const prev = map[u.date] || 0;
    const val = u.impact === "high" ? 3 : u.impact === "medium" ? 2 : 1;
    if (val > prev) map[u.date] = val;
  }
  return map;
}
...
type Tab = "updates" | "heatmap" | "locked";

/* ─── Component ─── */

export function PowContent() {
      ...
      <Container size="default">
        ...
        <div className="text-center">
          ...
          <p className="mx-auto mt-4 max-w-2xl text-ocean-mist">
            A record of what shipped and moved the project forward. Each update
            is tagged by area and color-coded by impact. No promises — only work
            done.
          </p>
        </div>

        {/* Tabs */}
        <div className="mt-10 flex justify-center gap-6 border-b border-ocean-mist/10">
          {([
            ...
            <button
              ...
              className={`pb-3 text-sm font-medium transition-colors ${
                tab === t.id
                  ? "border-b-2 border-icy-aqua text-icy-aqua"
                  : "text-blue-slate hover:text-ocean-mist"
              }`}
            >
              {t.label}
            </button>
  ...
  return (
    <div>
      ...
      <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <div className="rounded-card border border-ocean-mist/10 bg-dark-surface p-4 text-center">
          <div className="text-2xl font-bold text-icy-aqua">{UPDATES.length}</div>
          <div className="mt-1 text-xs text-blue-slate">Total updates</div>
        </div>
        <div className="rounded-card border border-ocean-mist/10 bg-dark-surface p-4 text-center">
          <div className="text-2xl font-bold text-aquamarine">
            {UPDATES.filter((u) => u.impact === "high").length}
          </div>
          <div className="mt-1 text-xs text-blue-slate">High impact</div>
        </div>
        <div className="rounded-card border border-ocean-mist/10 bg-dark-surface p-4 text-center">
          <div className="text-2xl font-bold text-soft-cyan">
            {new Set(UPDATES.map((u) => u.date)).size}
          </div>
          <div className="mt-1 text-xs text-blue-slate">Active days</div>
        </div>
        <div className="rounded-card border border-ocean-mist/10 bg-dark-surface p-4 text-center">
          <div className="text-2xl font-bold text-ocean-mist">
            {new Set(UPDATES.flatMap((u) => u.tags)).size}
          </div>
          <div className="mt-1 text-xs text-blue-slate">Categories</div>
        </div>
      </div>
    </div>
  );
...
function TokenLockedTab() {
  return (
    <div>
      <div className="rounded-card border border-ocean-mist/10 bg-dark-surface p-8">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
          ...
          <div className="text-center">
            ...
            <div className="mt-2 text-2xl font-bold text-ocean-mist">—</div>
            <div className="mt-1 text-xs text-blue-slate">To be announced</div>
          </div>
        ...
        <div className="mt-8 space-y-4">
          <div className="flex items-center justify-between rounded-lg border border-ocean-mist/5 bg-dark-elevated px-5 py-3.5">
            <span className="text-sm text-ocean-mist">Contract Address</span>
            <span className="font-mono text-sm text-blue-slate">
              Coming after launch
            </span>
          </div>
          <div className="flex items-center justify-between rounded-lg border border-ocean-mist/5 bg-dark-elevated px-5 py-3.5">
            <span className="text-sm text-ocean-mist">Lock Platform</span>
            <span className="font-mono text-sm text-blue-slate">TBA</span>
          </div>
          <div className="flex items-center justify-between rounded-lg border border-ocean-mist/5 bg-dark-elevated px-5 py-3.5">
            <span className="text-sm text-ocean-mist">Lock TX</span>
            <span className="font-mono text-sm text-blue-slate">
              Coming after launch
            </span>
          </div>
          <div className="flex items-center justify-between rounded-lg border border-ocean-mist/5 bg-dark-elevated px-5 py-3.5">
            <span className="text-sm text-ocean-mist">Unlock Date</span>
            <span className="font-mono text-sm text-blue-slate">TBA</span>
          </div>
        </div>

        <div className="mt-6 rounded-lg border-l-4 border-soft-cyan/30 bg-soft-cyan/5 px-4 py-3 text-sm text-ocean-mist">
          💡 Token lock details will be updated here immediately after the
          PumpFun relaunch. All lock transactions will be verifiable on-chain.
        </div>
      </div>
    </div>
  );
